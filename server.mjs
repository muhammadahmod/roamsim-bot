...
app.post("/api/webhook", async (req, res) => {
  ...
  if (lower === "topup" || lower === "top up" || lower === "top-up") {
    ...
    const order = findOrderByReference(ref);
    if (!order) {
      return void res.send(twimlMessage(`❌ No order found for *${ref || "(none provided)"}*.\n\nPlease check your reference and try again, e.g. *PAID ESIM-A3F9*.\n\nType *help* if you need assistance.`));
    }
    if (order.status === "fulfilled" || order.esim) {
      return void res.send(twimlMessage(`✅ Your eSIM for *${order.planName}* (ref *${order.reference}*) was already sent - check this chat or your email. Type *help* if you cannot find it.`));
    }
    if (order.status === "fulfilling") {
      return void res.send(twimlMessage(`⏳ Payment confirmed - your eSIM for *${order.reference}* is being prepared now and will arrive here shortly.`));
    }
    let tx = null;
    try { tx = await paystackVerify(order.reference); } catch (e) { console.error("paystackVerify (PAID) error", e); }
    if (!tx || tx.status !== "success") {
      return void res.send(twimlMessage(`⏳ I have not received your payment yet for *${order.reference}*.\n\nPlease complete payment using the secure link we sent - the moment it clears, your eSIM QR arrives here automatically. Type *help* if you need a hand.`));
    }
    updateOrderStatus(order.id, "fulfilling");
    res.send(twimlMessage(`✅ Payment confirmed - thank you! Preparing your *${order.planName}* eSIM now. Your QR code will arrive here in a few seconds. 🎉`));
    try {
      await fulfillOrder(order);
      notifyAdmin(`✅ Auto-fulfilled ${order.reference} (${order.planName} - ${order.customerName}) via PAID verify.`);
    } catch (e) {
      console.error("fulfillOrder (PAID) failed", e);
      notifyAdmin(`⚠️ Fulfilment FAILED for ${order.reference} after verified payment: ${e.message}. Reply FULFILL ${order.reference} to retry.`);
    }
    return;
  }
